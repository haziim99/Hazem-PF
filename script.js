    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < MAX_LINK_DIST) {
          ctx.beginPath();
          ctx.strokeStyle =
            "rgba(168,85,247," + 0.07 * (1 - dist / MAX_LINK_DIST) + ")";
          ctx.lineWidth = 0.5;
        ...
        if (match) {
          ...
          setTimeout(() => {
            card.style.transition =
              "opacity 0.45s ease, transform 0.45s cubic-bezier(.4,0,.2,1)";
...
const PROJECT_DATA = {
  ...
  p4: {
    ...
    desc: "A premium, interactive driving exam preparation platform built with Angular and Java Spring Boot. Features reactive state management, a multi-language micro-translation engine, secure Firebase authentication, and cinematic UI animations.",
    features: [
      "Interactive exam simulation and training modules (Uebung & Zeitdruck)",
      "Custom translation engine implemented with Angular Signals supporting DE, AR, and EN",
      "Secure authentication and data interceptors powered by Google Firebase",
      "Cinematic landing page animations using GSAP and ScrollTrigger",
      "Robust Java Spring Boot backend architecture with custom CORS configuration",
      "Fully automated CI/CD deployment pipeline via GitHub Pages",
    ],
    stack: [
      { label: "Angular", cls: "tag-angular" },
      { label: "Spring Boot", cls: "tag-springboot" },
      { label: "Java", cls: "tag-java" },
      { label: "Firebase", cls: "tag-firebase" },
      { label: "GSAP", cls: "tag-gsap" },
    ],
  },
};
...
function initContactForm() {
  ...
  nameInput &&
    nameInput.addEventListener("blur", () =>
      validateField(nameInput, "err-name", nameInput.value.trim().length > 0),
    );
  emailInput &&
  ...
  msgInput &&
    msgInput.addEventListener("blur", () =>
      validateField(msgInput, "err-msg", msgInput.value.trim().length > 0),
    );
